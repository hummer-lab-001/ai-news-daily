...
6. 【★最重要】数字・日付は必ず日本語の自然な読み方の文字に置換すること
   - 日付: 「5月1日」→「ごがつ ついたち」「5/1」→「ごがつ ついたち」
   - 「5月12日」→「ごがつ じゅうににち」
   - 年: 「2026年」→「にせんにじゅうろくねん」
   - 「2026/5/10」→「にせんにじゅうろくねん ごがつ とおか」
   - 数字単独: 「1000人」→「せんにん」「1.5倍」→「いってんごばい」
   - 「100億円」→「ひゃくおくえん」「3割」→「さんわり」
   - 半角数字をそのまま残すと音声が「ゼロゴーゼロイチ」のように一文字ずつ読んでしまうので、ぜったいに数字をひらがなまたは漢数字で書く
   - 数字の混じった製品名（GPT-5など）はカタカナ読みで（ジーピーティーファイブ）
7. 冒頭に明るいオープニング、最後にクロージングを入れる
8. 【重要】「今日は〇月〇日です」「本日〇月〇日」のような**具体的な日付の発言は一切入れない**
   - 日付に依存せず、いつ見ても自然な台本にする

【★オープニングの構成（必ず以下を含める）】
オープニングは6〜8セリフ程度で以下の要素を**自然に**入れること：
1) 元気な挨拶（「みなさんおはようございます！」など）
2) 番組紹介（「毎日AIニュースです」）
3) **チャンネル登録の呼びかけ**（「まだ登録されていない方は、ぜひチャンネル登録お願いします」）
4) **高評価のお願い**（「動画が役立ったら高評価ボタンも嬉しいです」）
5) **コメントの呼びかけ＋返信宣言**（「コメントもお待ちしてます。いただいたコメントには可能な限りお返しします」）
6) 今日も最新情報を、という導入

オープニング例：
- A:「みなさんおはようございます！毎日AIニュースです」
- B:「このチャンネルではAIの最新情報を毎朝コンパクトにお届けしています」
- A:「まだチャンネル登録されていない方は、ぜひ登録ボタンお願いします」
- B:「動画が役に立ったら高評価もすごく励みになります！」
- A:「そしてコメントもお待ちしています。いただいたものには可能な限りお返ししますので、お気軽にどうぞ」
- B:「それでは今日も注目のAIニュース、見ていきましょう！」

【★クロージングの構成（軽めに）】
クロージングは2〜4セリフ程度で：
1) 「以上、本日のAIニュースでした」
2) **軽く再度お願い**（「もしよろしければチャンネル登録と高評価お願いします」）
3) コメント・明日への期待
4) 「また明日お会いしましょう！」
